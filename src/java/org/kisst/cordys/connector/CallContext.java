...
public class CallContext extends CallTrace {
	...
	protected final CallTimer timer;
	...
	public int callMethod(int method, String resultVar, boolean ignoreSoapFault) {
    	...
    	try {
			...
			getBaseConnector().logPerformance("CALL", this, startTime, method, succes);
			mon1.stop();
			mon2.stop();
			monu1.stop();
			monu2.stop();
	...
	public void checkAndLogResponse(int response, String resultVar, boolean ignoreSoapFault) {
		...
		if ((! ignoreSoapFault) && SoapUtil.isSoapFault(responseBody)) {
				trace(Severity.WARN, new CallTrace.Item("Result of methodcall for "+resultVar+" returned Fault: ",responseBody));
